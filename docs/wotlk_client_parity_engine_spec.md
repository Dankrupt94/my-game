...
- Treat `stage17_vendor_view.tscn` as a vendor list plus bounded buy/sell proof until normal row-selected buy/sell UI, repair, item metadata, stock refresh, failure-code handling, and persistent inventory refresh behavior are implemented and validated.
