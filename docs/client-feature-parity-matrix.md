...
| Inventory | Working expanded Stage 17 slice | Player private update fields for equipped slots, bag slots, backpack slots, item GUIDs, coinage field detection, item object entry/stack/durability fields, and `CMSG_ITEM_QUERY_SINGLE` / `SMSG_ITEM_QUERY_SINGLE_RESPONSE` template lookup. | `scenes/stage17_inventory_view.tscn` with 39-slot equipment/bag/backpack grid showing live item names, entries, stack counts, and durability when present. | `ACORE_INVENTORY_SELF_TEST=1 godot-4 --headless --path . res://scenes/stage17_inventory_view.tscn` returns 39 slots, 7 populated slots, 7 item-detail rows, and 7 resolved names for `Codexstage`. | Read-only inventory slot details now work through the Godot client path. Remaining inventory parity includes item icons/tooltips/stats, bag contents beyond backpack, split/swap/use/destroy packets, loot handoff, equipment mutation, and persistence checks after item actions. |
