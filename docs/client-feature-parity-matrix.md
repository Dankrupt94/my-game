...
| Inventory | Working expanded Stage 17 slice | Player private update fields for equipped slots, bag slots, backpack slots, item GUIDs, coinage field detection, item object entry/stack/durability fields, `CMSG_ITEM_QUERY_SINGLE` / `SMSG_ITEM_QUERY_SINGLE_RESPONSE` template lookup, controlled `CMSG_SWAP_INV_ITEM` base-backpack move/restore, and controlled `CMSG_SPLIT_ITEM` stack split/merge. | `scenes/stage17_inventory_view.tscn` with 39-slot equipment/bag/backpack grid showing live item names, entries, stack counts, durability when present, plus test controls for backpack move, equipment unequip, and stack split. | `ACORE_INVENTORY_SELF_TEST=1 godot-4 --headless --path . res://scenes/stage17_inventory_view.tscn` returns 39 slots, 7 populated slots, 7 item-detail rows, and 7 resolved names for `Codexstage`. `ACORE_INVENTORY_SWAP_SELF_TEST=1` confirms swap/restore for slots `23` and `25`. `ACORE_STACK_SPLIT_SELF_TEST=1` confirms split/merge for slots `23` and `25`. | Inventory slot details, reversible server-side item movement, and a reversible stack split/merge now work through the Godot client path. Remaining inventory parity includes item icons/tooltips/stats, bag contents beyond backpack, player drag/drop, item use/destroy/equip variants, loot handoff, item cooldown/failure UI, nested bag mutation, and broader persistence checks after item actions. |
