...
| Action bars | Working expanded slice | `SMSG_ACTION_BUTTONS` parser, 144 packed action slots, action id/type unpacking, spell-slot casting through `CMSG_CAST_SPELL`. | `scenes/stage16_action_bar_view.tscn` with all 144 slots, populated-slot details, clickable spell slots, and target controls for the current cast probe. | `ACORE_ACTION_BAR_SELF_TEST=1 godot-4 --headless --path . res://scenes/stage16_action_bar_view.tscn` returns 144 slots and 3 populated slots for `Codexstage`. `ACORE_ACTION_BAR_CAST_SELF_TEST=1 godot-4 --headless --path . res://scenes/stage16_action_bar_view.tscn` casts server action button `73` spell `78` and receives opcode `0x131`. | Server-provided action buttons display and one populated spell button can cast through the Godot client path. Remaining action-bar parity includes names/icons, drag/drop placement, keybinds, `CMSG_SET_ACTION_BUTTON`, persistence checks, macros, items, equipment sets, paging, and non-spell actions. |
