...
| Spells | Working first slice | `SMSG_INITIAL_SPELLS` parser, initial spell IDs, cooldown count. | `scenes/stage16_spellbook_view.tscn` with spell list. | `ACORE_SPELLBOOK_SELF_TEST=1 godot-4 --headless --path . res://scenes/stage16_spellbook_view.tscn` returns 48 initial spells for `Codexstage`. | Read-only spellbook works. Remaining spell parity includes names/icons/ranks, cast packets, target flags, cooldown updates, interrupt/fail messages, auras, and combat results. |
