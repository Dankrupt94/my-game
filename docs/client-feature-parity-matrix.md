...
| Inventory | Working expanded Stage 17 slice | Player private update fields for equipped slots, bag slots, backpack slots, item GUIDs, coinage field detection, item object entry/stack/durability fields, `CMSG_ITEM_QUERY_SINGLE` / `SMSG_ITEM_QUERY_SINGLE_RESPONSE` template lookup, and a controlled `CMSG_SWAP_INV_ITEM` base-backpack move/restore probe. | `scenes/stage17_inventory_view.tscn` with 39-slot equipment/bag/backpack grid showing live item names, entries, stack counts, durability when present, and a test move button for backpack slot `23` to slot `25` with restore. | `ACORE_INVENTORY_SELF_TEST=1 godot-4 --headless --path . res://scenes/stage17_inventory_view.tscn` returns 39 slots, 7 populated slots, 7 item-detail rows, and 7 resolved names for `Codexstage`. `ACORE_INVENTORY_SWAP_SELF_TEST=1 godot-4 --headless --path . res://scenes/stage17_inventory_view.tscn` confirms swap and restore for slots `23` and `25`. | Inventory slot details and the first reversible server-side item movement now work through the Godot client path. Remaining inventory parity includes item icons/tooltips/stats, bag contents beyond backpack, player drag/drop, split/use/destroy/equip packets, loot handoff, item cooldown/failure UI, nested bag mutation, and broader persistence checks after item actions. |
