...
func questgiver_reward_probe(
	...
	host: String = "127.0.0.1",
	port: String = "3724") -> Dictionary:
	return questgiver_reward_probe_selector(character_name, str(target_entry), quest_id, target_name, host, port)


func questgiver_reward_probe_selector(
	character_name: String = "Codexstage",
	target_selector: String = "823",
	quest_id: int = 6,
	target_name: String = "Nearby Quest Giver",
	host: String = "127.0.0.1",
	port: String = "3724") -> Dictionary:
	var native_result := _run_native_questgiver_reward_probe_selector(character_name, target_selector, quest_id, target_name, host, port)
	if not native_result.is_empty():
		return native_result
	...
	var exit_code := _execute_helper_with_password(
		...
		output)
	var text := "\n".join(output)
	var parsed := _parse_questgiver_reward_output(text)
	parsed["exit_code"] = exit_code
	parsed["output"] = text.strip_edges()
	parsed["source"] = "helper process"
	parsed["ok"] = exit_code == 0 \
		and (bool(parsed.get("offer_reward_seen", false)) or bool(parsed.get("request_items_seen", false))) \
		and bool(parsed.get("quest_removed_after_remove", false))
	return parsed
...
func _run_native_questgiver_reward_probe_selector(
	character_name: String,
	target_selector: String,
	quest_id: int,
	target_name: String,
	...
	var client: Object = credentials["client"]
	if not client.has_method("questgiver_reward_probe_selector"):
		return {}

	var result = client.call(
		"questgiver_reward_probe_selector",
		host,
		port,
		credentials["account"],
		credentials["password"],
		...
		target_name)
	if typeof(result) != TYPE_DICTIONARY:
		return _failure("Native Godot protocol client returned an unexpected questgiver reward result")

	var reward_parsed: Dictionary = result
	reward_parsed["source"] = "Godot native extension"
	reward_parsed["exit_code"] = 0 if bool(reward_parsed.get("ok", false)) else 1
	reward_parsed["output"] = JSON.stringify(_redacted_result(reward_parsed))
...
func _parse_questgiver_reward_output(output: String) -> Dictionary:
	var result := {
		"auth_flow_ok": false,
		"live_target_found": false,
		"questgiver_hello_sent": false,
	...
	for raw_line in output.split("\n"):
		...
		if line.begins_with("AUTH_FLOW_OK"):
			result["auth_flow_ok"] = true
			result["realm_line"] = line
		elif line.begins_with("QUESTGIVER_REWARD_PROBE"):
			result["character_name"] = _extract_quoted_field(line, "character=\"")
			result["target_guid"] = _extract_token_after(line, "target_guid=")
			result["target_entry"] = _extract_int_field(line, "target_entry=")
			result["quest_id"] = _extract_int_field(line, "quest_id=")
			...
			result["quest_invalid_seen"] = _extract_int_field(line, "quest_invalid_seen=") == 1
			result["response_opcode"] = _extract_hex_field(line, "response_opcode=0x")
